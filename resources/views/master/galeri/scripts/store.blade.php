<script>
    $(document).ready(function() {
        ...
        handleFormGaleri(
            '#createForm',
            '{{ route('master-data.galeri.store') }}',
            'POST',
    ...
    function storeGaleriData(form) {
        const formData = new FormData();

        formData.append('tanggal', form.find('#tanggal').val());
        formData.append('kegiatan', form.find('#kegiatan').val());
        ...
        if (fileInput.files.length === 0) {
            $('#modalError').removeClass('d-none').text('Foto galeri wajib diunggah.');
    ...
    function handleFormGaleri(formSelector, url, method, successMessage, getDataFn, isMultipart = false) {
